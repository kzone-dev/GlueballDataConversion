...
function resample_full_correlator(file_in, file_out; n_batch = 100, save_vev = false)
    f1 = h5open(file_in,"r")
    ...
    if save_vev
        vev = f1["full_vev"]
        resample_vev = zeros(n_batch,Nops)
        resample_vev_sum = zeros(Nops)
    end
    ...
    create_dataset(f2, "vev_resample", Float64, (Nmeas,Nops))
    ...
    @showprogress for n in Iterators.partition(1:Nmeas, n_batch)
        ...
        if save_vev
            resample_vev_sum .= resample_vev_sum .+ dropdims(sum( vev[n,:],dims=1),dims=1) 
        end
    end
    ...
    @showprogress for n in Iterators.partition(1:Nmeas, n_batch)
        ...
        if save_vev
            batch_vev = vev[n,:]
            @inbounds for ind2 in 1:Nops, ind1 in eachindex(n)
                resample_vev[ind1,ind2] = resample_vev_sum[ind2] - batch_vev[ind1,ind2]
            end
            f2["vev_resample"][n,:] = resample_vev[eachindex(n),:]
        end
...
resample_full_correlator(file_in, file_out, save_vev=true)